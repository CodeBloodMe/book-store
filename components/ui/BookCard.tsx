...
export default function BookCard({ book, featured = false }: BookCardProps) {
  ...
  useEffect(() => {
    ...
    // Lazy load fast-average-color to keep initial JS bundle small
    import('fast-average-color').then(({ FastAverageColor }) => {
      const fac = new FastAverageColor();
      const img = new window.Image(); // Use native Image object for color extraction
      img.crossOrigin = 'anonymous';
      img.src = coverUrl;
      img.onload = () => {
        fac.getColorAsync(img, { algorithm: 'dominant' })
          .then(color => {
            // Use the pure, vibrant color
  ...
  return (
    ...
    >
      ...
      <Link href={`/books/${book.id}`} className="absolute inset-0 z-10" aria-label={`View ${book.title}`} />
      ...
      <div className="absolute inset-0 bg-gray-900 pointer-events-none">
        ...
        />
        {/* Extra dark gradient at the very bottom for text contrast */}
      ...
      <div className="absolute bottom-0 left-0 right-0 p-5 md:p-6 flex flex-col justify-end z-20 pointer-events-none">
        ...
        <p className="text-gray-300 text-sm leading-relaxed line-clamp-3 mb-4">
          {book.description ? (
            <>
              <Link href={`/authors/${encodeURIComponent(book.author)}`} className="font-bold text-white hover:underline relative z-30 pointer-events-auto">
                {book.author}
              </Link>
              {' — '}{book.description}
            </>
